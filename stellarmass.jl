...
galgroups = DataFrame(
	CSV.File(
		"galaxies_saulder.dat",
		delim = ' ',
		ignorerepeated = true,
		header = [:galID, :grpID, :name, :RA, :Dec, :z, :logMstar],
	))
...
groups = DataFrame(
	CSV.File(
		...
		delim = ' ',
		ignorerepeated = true,
		header = [:grpID, :RA, :Dec, :z, :logLtot, :e_logLtot, :logLobs,
			:logMtot, :e_logMtot, :logMstar, :NMstar, :logMdyn, :sigma,
			:Rad, :angRad, :DL, :Ntot],
	))
...
galaxies = DataFrame(
	CSV.File(
		...
		ignorerepeated = true,
		comment = "#",
		header = [:galID, :RAdeg, :DECdeg, :l, :b, :k_c, :h_c, :j_c,
			:k_tc, :h_tc, :j_tc, :e_k, :e_h, :e_j, :e_kt, :e_ht, :e_jt,
			:e_bv, :r_iso, :r_ext, :ba, :flgs, :type, :ts, :v, :e_v, :c,
			:vsrc, :CAT_ID],
	)
)
select!(galaxies, Not(:Column30))
end

# ╔═╡ 237f3c87-89fd-4e19-9fce-b56c1519653b
gal_Mstar = filter(row -> row.logMstar != -99.99, galgroups) # Remove galaxies that don't have stellar mass

# ╔═╡ 9114160f-a0ee-4459-8fa6-4c3ac12242a4
length(Set(gal_Mstar.grpID))

# ╔═╡ 806fea85-07a2-4dd1-ba8a-f03e5e3001c1
begin
	galgrp = innerjoin(gal_Mstar, groups, on = :grpID, renamecols = "_gal"=>"_grp")
	galfull = innerjoin(galgrp, galaxies, on = :name_gal=>:galID) 
	galfull.grpIDcat = CategoricalArray(galfull.grpID)
	filter!(row -> row.NMstar_grp >= 10, galfull) # Only use groups with at least 10 galaxies with stellar mass
end

# ╔═╡ 195153dd-6225-447f-82ed-89c6584f85c5
m = fit(MixedModel, @formula(logMstar_gal ~ 1 + logLtot_grp + logMstar_grp + Ntot_grp + k_tc + (1|grpIDcat)), galfull)
